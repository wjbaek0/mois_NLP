...
# 모델 로그 저장을 위한 로깅 파일 호출 
	with open(Config.LOGGING_FILE, 'a+t') as log:
		...
		# 학습 진행도 log , acc 정확도를 보기 위한 체크포인트 
...
if __name__=='__main__':
	...
	# 데이터셋을 model.py와 동일 폴더에 (경로수정가능)
	dataset_train, dataset_val , dataset_test = get_data('ratings_train_8c.csv','ratings_valid_8c.csv', 'ratings_test_8c.csv') # csv 데이터 불러오기 
	...
	if Task == 1 :
		...
		pre_model.load_state_dict(torch.load('./kobert_model_20.pth'), strict=False) # Weight 경로확인
		...
		print("새로운 학습 체크포인트를 동일 폴더에 저장하여야 실행됩니다.")
