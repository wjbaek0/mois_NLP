...
if __name__=='__main__':
	...
	dataset_train, dataset_val , dataset_test = get_data('ratings_train.csv','ratings_valid.csv''ratings_test_mini.csv') # csv 데이터 불러오기 
	...
	if Task == 1 :
		...
		pre_model = BertModel.from_pretrained("skt/kobert-base-v1", return_dict=False)
		pre_model.load_state_dict(torch.load('./kobert_model_10.pth'), strict=False) # Weight 경로확인
		pre_model = BERTClassifier(pre_model, dr_rate=drop_rate).to(device)
