...
class BERTDataset(Dataset):  # dataset to bert_tokenizer
    def __init__(self, dataset, sent_idx, label_idx, bert_tokenizer,vocab, max_len, pad, pair):
        ...
        filtered_dataset = list(filter(lambda i: isinstance(i[0], str), dataset))
        # self.sentences = list(map(lambda i: transform(i[sent_idx]), filtered_dataset))
        self.sentences = [transform([i[sent_idx]]) for i in filtered_dataset] ## SENTENCES

        self.labels = [np.int32(i[label_idx]) for i in filtered_dataset] ## LABAL
...
# 모델 로그 저장을 위한 로깅 파일 호출 
	with open(Config.LOGGING_FILE, 'a+t') as log:
		...
		for e in range(num_epochs):
			...
			BEST_MODEL_PATH = os.path.join(Config.MODEL_PATH, 'kobert_model_{}.pth'.format(e))
...
def get_data(ratings_train,ratings_vaild,ratings_test):
	...
	train_data.loc[(train_data['label'] == "train"), 'label'] = 2  # 철도교통사고 => 2
	train_data.loc[(train_data['label'] == "ship"), 'label'] = 3  # 해양선박사고 => 3
	# train_data.loc[(train_data['label'] == "aircraft"), 'label'] = 4  # 항공기사고 => 4
	# train_data.loc[(train_data['label'] == "road"), 'label'] = 5  # 도로교통사고 => 5
	train_data.loc[(train_data['label'] == "chemical"), 'label'] = 6  # 유해화학물질사고 => 6
	# train_data.loc[(train_data['label'] == "infect"), 'label'] = 7  # 감염병 => 7

	val_data.loc[(val_data['label'] == "Construction"), 'label'] = 0  # 공사장 => 0
	val_data.loc[(val_data['label'] == "Fire"), 'label'] = 1  # 화재 => 1
	val_data.loc[(val_data['label'] == "train"), 'label'] = 2  # 철도교통사고 => 2
	val_data.loc[(val_data['label'] == "ship"), 'label'] = 3  # 해양선박사고 => 3
	# val_data.loc[(val_data['label'] == "aircraft"), 'label'] = 4  # 항공기사고 => 4
	# val_data.loc[(val_data['label'] == "road"), 'label'] = 5  # 도로교통사고 => 5
	val_data.loc[(val_data['label'] == "chemical"), 'label'] =  6  # 유해화학물질사고 => 6
	# val_data.loc[(val_data['label'] == "infect"), 'label'] = 7  # 감염병 => 7

	test_data.loc[(test_data['label'] == "Construction"), 'label'] = 0  # 공사장 => 0
	test_data.loc[(test_data['label'] == "Fire"), 'label'] = 1  # 화재 => 1
	test_data.loc[(test_data['label'] == "train"), 'label'] = 2  # 공사장 => 0
	test_data.loc[(test_data['label'] == "ship"), 'label'] = 3  # 해양선박사고 => 3
	# test_data.loc[(test_data['label'] == "aircraft"), 'label'] = 4  # 항공기사고 => 4
	# test_data.loc[(test_data['label'] == "road"), 'label'] = 5  # 도로교통사고 => 5
	test_data.loc[(test_data['label'] == "chemical"), 'label'] = 6 # 유해화학물질사고 => 6
	# test_data.loc[(test_data['label'] == "infect"), 'label'] =  7  # 감염병 => 7
...
def predict(model,test_dataloader):
	...
	# print("정답 라벨 리스트 : ", y_true_sort)
	# print(">> 추론된 값은 :  " , y_pred )
	# print(">> 추론값의 confidence :  " , confidence )
	pred_txt = [] # 틀린것 
	pred_path = "predicts"
	print(len(y_true_sort))
	for index,(i,j) in enumerate(zip(y_true_sort,y_pred)):
		if i == j :
			# pred_txt.append(f"{i}, {j}\n")
			cnt += 1
		else :
			pred_txt.append(f"index = {index+2}, y_true = {i}, y_pred = {j}\n")
   
	os.makedirs(pred_path, exist_ok=True)
	with open (pred_path + "/pred.txt", 'w') as f:
		f.writelines(pred_txt)
	
	print("추론 정확도 >>>> : ", round((cnt/len(y_pred))*100, 2) , " %")

##

	# classes = ('construct','fire','train','ship','aircraft','road','chemical','infect')
	classes = ('construct','fire','train','ship','chemical')
...
if __name__=='__main__':
	...
	dataset_train, dataset_val , dataset_test = get_data('Classification/ratings_train.csv','Classification/ratings_val.csv', 'Classification/ratings_test.csv') # csv 데이터 불러오기 
	...
	train_dataloader = torch.utils.data.DataLoader(data_train, batch_size=batch_size, num_workers=0) # cpu = 0
	valid_dataloader = torch.utils.data.DataLoader(data_val, batch_size=batch_size, num_workers=0) # cpu = 0
	test_dataloader = torch.utils.data.DataLoader(data_test, batch_size=batch_size, num_workers=0) # cpu = 0
	...
	if Task == 1 :
		...
		pre_model = BERTClassifier(model, dr_rate=drop_rate).to(device)
		# pre_model.load_state_dict(torch.load('./Classification/kobert_model_1.pth'), strict=False) # Weight 경로확인
		pre_model.load_state_dict(torch.load('./Classification/kobert_model_49.pth'), strict=False) # Weight 경로확인
	
		####### 추론 함수 호출 ###############
		predict(pre_model,test_dataloader)
		...
		checkpoint = torch.load('./Classification/kobert_model_DenseNet_ckpt.pt') # ckpt 경로 확인
