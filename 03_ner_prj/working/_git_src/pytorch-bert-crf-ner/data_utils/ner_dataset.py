...
class NamedEntityRecognitionDataset(Dataset):
    ...
    def load_data_from_txt(self, file_full_name):
        with codecs.open(file_full_name, "r", "utf-8") as io:
            ...
            for line in lines:
                if prev_line == "\n" or prev_line == "" or prev_line.isspace():
                    save_flag = True
...
class NamedEntityRecognitionFormatter():
    """ NER formatter class """
    def __init__(self, vocab=None, tokenizer=None, maxlen=30, model_dir=Path('data_in'), model_type='kobert'):
        ...
        if vocab is None or tokenizer is None:
            ...
            if model_type == 'kcbert':
                pre_tokenizer = AutoTokenizer.from_pretrained("beomi/kcbert-base")
                vocab_of_gluonnlp = AutoTokenizer.from_pretrained("beomi/kcbert-base")
                token2idx = vocab_of_gluonnlp.vocab
                ...
                pre_tokenizer = KoBERTTokenizer.from_pretrained('skt/kobert-base-v1')
                vocab_of_gluonnlp = nlp.vocab.BERTVocab.from_sentencepiece(pre_tokenizer.vocab_file, padding_token='[PAD]')
    ...
    def transform_source_fn(self, text):
        ...
        text = re.sub(' +', ' ', text)[1:] if text[0].isspace() else re.sub(' +', ' ', text)
    ...
    def transform_target_fn(self, label_text, tokens, prefix_sum_of_token_start_index):
        ...
        :return:
        """
        label_text = re.sub(' +', ' ', label_text)[1:] if label_text[0].isspace() else re.sub(' +', ' ', label_text)
