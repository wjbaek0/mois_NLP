...
def append_process(sentences: List[Sentence], summaries: List[Sentence], substr: List[str]) -> List[Sentence]:
    for s in [sent for sent in sentences if sent not in summaries]:
        if any(x in s.text for x in substr):
            summaries.append(s)


def remove_process(summaries: List[Sentence], substr: List[str]) -> List[Sentence]:
    to_delete = []
    for i in range(len(summaries)):
        if any(x in summaries[i].text for x in substr):
            to_delete.append(i)
...
def _get_reordered_summary_texts(summaries: List[Sentence]) -> List[str]:
    ...
    reordered_summary_texts: List[str] = [
        sentence.text for sentence in summaries]
    extracted_sentences_ids: List[str] = [
        sentence.index for sentence in summaries]
    summary_length: List[int] = [len(summaries)]
    texts_and_ids: Dict = {
        'text': reordered_summary_texts, 'ids': extracted_sentences_ids, 'length': summary_length}
...
class LexRank:
    ...
    def __init__(self,
        ...
        if self.similarity_method == 'tfidf_cosine':
            ...
            raise LexRankError(
                "Available similarity methods are: 'tfidf_cosine', 'jaccard_index', 'normalized_cooccurrence'.")

        # clustering model init
        self._clustering_model: Optional[Union[Birch,
                                               DBSCAN, AffinityPropagation]]
        if clustering_method == 'birch':
            self._clustering_model = Birch(
                threshold=self.birch_threshold, n_clusters=self.birch_n_clusters)
            ...
            raise LexRankError(
                "Available clustering methods are: 'birch', 'dbscan', 'affinity_propagation', or None (no clustering).")
    ...
    def _similarity_function_tfidf_cosine(self, sentence1: Sentence, sentence2: Sentence) -> float:
        if sentence1.index == sentence2.index:
            return 1.0
        sentence1_tfidf_dict: Dict[int, float] = {
            word_id: tfidf for word_id, tfidf in sentence1.tfidf_pairs}
        sentence2_tfidf_dict: Dict[int, float] = {
            word_id: tfidf for word_id, tfidf in sentence2.tfidf_pairs}
        tfidf_vec1, tfidf_vec2 = self._dict_vectorizer.fit_transform(
            [sentence1_tfidf_dict, sentence2_tfidf_dict]).toarray()
    ...
    def _similarity_function_normalized_cooccurrence(self, sentence1: Sentence, sentence2: Sentence) -> float:
        ...
        q: float = np.log(len(sentence1.tokens)) + \
            np.log(len(sentence2.tokens))
        try:
            return p / q
        except ZeroDivisionError:
            return 0.0
    ...
    def summarize(self,
        ...
        self.sentences: List[Sentence] = parse_text_into_sentences(
            text, self.tokenizer)
        ...
        if self.similarity_method == 'tfidf_cosine':
            ...
            self.tfidf_model: TfidfModel = TfidfModel(
                self.corpus.bows, id2word=self.corpus.dictionary, normalize=True)
        ...
        if self.prune_duplicates:
            ...
            for cluster in self.clusters:
                ...
                duplication_table: Dict[int, bool] = {
                    i: False for i in range(cluster_size)}
        ...
        # organize clusters
        self.clusters = [cluster for cluster in self.clusters if len(
            cluster) >= self.min_cluster_size]
        self.clusters = sorted(self.clusters, key=len, reverse=True)
        self.num_clusters: int = len(self.clusters)
        # the number of effective sentences
        self.max_k = sum([len(cluster) for cluster in self.clusters])
        ...
        for i, cluster in enumerate(self.clusters):
            ...
            for sentence1 in cluster:
                for sentence2 in cluster:
                    weight: float = self.matrix[sentence1.index,
                                                sentence2.index]
                    if weight:
                        graph.add_edge(sentence1, sentence2, weight=weight)
            sentence2prob: Dict[Sentence, float] = pagerank(
                graph, weight='weight')
            self.clusters[i] = sorted(
                sentence2prob, key=sentence2prob.get, reverse=True)
    ...
    def probe(self, k: Union[int, float] = 0) -> List[str]:
        ...
        if not k:
            ...
            raise LexRankError(
                "An appropriate value for `k` is float(0~1) for compress rate, or a int(>1) for the exact number of sentences.")
        elif k > self.max_k:
        ...
        # prepare probing
        self.summaries: List[Sentence] = []
        cid2sizes: np.ndarray = np.array(
        ...
        for cid, cluster in enumerate(self.clusters):
            self.summaries.append(cluster[0])
            cid2steps[cid] += 1
            if len(self.summaries) == effective_k:

                append_substr = ['재산', '인명', '원인', '재 산', '인 명', '원 인', '일시', '일 시', '시간', '시 간', '장소', '장 소', '소재지', '손실', '년']
                append_process(self.sentences, self.summaries, append_substr)

                remove_substr = ['http', 'htm', 'v=', 'www']
                remove_process(self.summaries, remove_substr)

                return _get_reordered_summary_texts(self.summaries)

        # after the first round: all clusters should contribute about equal amount of sentences in the final summary
        while True:
            cid2ahead: np.ndarray = np.array(
                [cid2steps + 1, cid2sizes]).min(axis=0) / cid2sizes  # +1 denotes "the next step"
            selected_cid: int = int(cid2ahead.argmin())
            current_step: int = int(cid2steps[selected_cid])
            # fail-safe (this should never happend)
            if current_step >= int(cid2sizes[selected_cid]):
                break
            self.summaries.append(self.clusters[selected_cid][current_step])
