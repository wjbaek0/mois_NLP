...
def main() -> None:
    ...
    if command == Command.Train:
        ...
        val_results_2 = trainer.test(test_dataloaders=task.val_loader, verbose=False)
        print(val_results_2)
        print("-" * 80)
        ...
        import torch
        model = task.model
        model.load_state_dict(torch.load("D:/mois-workspace/KLUE/KLUE-baseline/output/klue-dp/version_28/transformers/pytorch_model.bin"))        
        trainer.test(model, test_dataloaders=task.test_loader)
